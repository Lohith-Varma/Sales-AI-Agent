...
import { Bell, BrainCircuit, Check, DatabaseZap, LockKeyhole, Moon, SlidersHorizontal, Sun, UserRound, Volume2 } from "lucide-react";
...
export function SettingsPage() {
  ...
  return <div className="space-y-7"><PageHeader eyebrow="Workspace configuration" title="Settings" description="Interface and AI-display preferences are saved on this device. Server profile, integrations, notifications, voice, and security settings require backend APIs." />
      ...
      <div className="space-y-5"><Card className="p-5"><p className="flex items-center gap-2 text-sm font-semibold"><UserRound className="size-4 text-blue-600" />Profile</p><div className="mt-4 flex items-center gap-3"><div className="grid size-11 place-items-center rounded-xl bg-slate-900 text-xs font-semibold text-white">SA</div><div><p className="text-sm font-medium">Sales Agent</p><p className="text-xs text-slate-400">Unauthenticated workspace</p></div></div><p className="mt-4 rounded-xl bg-amber-50 p-3 text-xs leading-5 text-amber-800 dark:bg-amber-950/40 dark:text-amber-300">No current-user or profile endpoint exists. Display name is not identity.</p></Card><Card className="p-5"><p className="flex items-center gap-2 text-sm font-semibold"><Bell className="size-4 text-blue-600" />Notifications</p><Separator className="my-4" /><Toggle checked={false} onCheckedChange={() => undefined} label="Call alerts" description="Requires notification preferences API." disabled /><Toggle checked={false} onCheckedChange={() => undefined} label="Follow-up reminders" description="Requires reminders and notifications APIs." disabled /></Card><Card className="p-5"><p className="flex items-center gap-2 text-sm font-semibold"><LockKeyhole className="size-4 text-red-600" />Security</p><p className="mt-3 text-xs leading-5 text-slate-500">Login, sessions, MFA, password, and role enforcement are absent from the backend.</p><div className="mt-3 flex items-center gap-2 text-xs font-medium text-red-600"><DatabaseZap className="size-3.5" />Production blocker</div></Card><Card className="p-5"><p className="flex items-center gap-2 text-sm font-semibold"><Volume2 className="size-4 text-blue-600" />Audio format</p><p className="mt-3 text-xs leading-5 text-slate-500">Live microphone streaming uses the backend-required mono 16 kHz PCM16 format.</p></Card></div></div>
  </div>;
}
